<?php
$data=[];
$filename = isset($argv[1]) ? $argv[1] : 'dekstra_report.txt';
$filenameOut = isset($argv[2]) ? $argv[2] : 'dekstra_report.csv';
$delimetr = ';';
$handle = fopen($filename, 'r');
if ($handle) {
    while (($line = fgets($handle)) !== false) {
        $matches=[];		
        preg_match('/.*dekstra_(\d+)_proccessor_(\d+): (\d+)ms.*/i',$line,$matches);
        if (!empty($matches)){
